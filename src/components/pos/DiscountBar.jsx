...
export default function DiscountBar({ subtotal }) {
  ...
  async function applyCode() {
    ...
    try {
      ...
      setDiscount(Object.assign({}, data, { calculatedAmt: Math.round(amt) }))
      setCode('')
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-green-800">{discount.code} - {discount.name}</p>
          <p className="text-xs text-green-600">- {formatMMK(discount.calculatedAmt)} off</p>
        </div>
